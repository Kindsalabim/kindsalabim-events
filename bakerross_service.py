...
def compute_kundenpreis(br_preis, faktor):
    if br_preis is None:
        return None
    try:
        return round(float(br_preis) * float(faktor), 2)
...
def fetch_price(url):
    """BR-Preis (€) von der Produktseite. None bei Fehler. Reihenfolge: JSON-LD →
    data-price-amount (kleinster plausibler = Ab-Preis) → Magento-Config."""
    try:
...
def ensure_price(db, produkt):
    """Liefert den BR-Preis; lädt ihn nach, wenn fehlend oder älter als 7 Tage."""
    frisch = False
...
def kurate(db, query, max_results=12, faktor=2.5):
    ...
    # 3. Preise nachladen (nur für die wenigen Treffer, respektvoll mit kleiner Pause)
    treffer = []
    for i, (p, grund) in enumerate(ausgewaehlt):
        ...
        treffer.append({
            "produkt": p,
            "br_preis": br_preis,
            "kundenpreis": compute_kundenpreis(br_preis, faktor),
            "grund": grund,
        })
    return treffer
